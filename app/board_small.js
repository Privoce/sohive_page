(function () {
  "use strict";

  // 防止重复加载
  if (window.SohiveWidget) {
    console.warn("Sohive Widget 已经加载");
    return;
  }

  // Widget 类
  class SohiveWidget {
    // 可配置的 API 信息
    statusBaseUrl = "https://api.sohive.com";
    // 使用用户指定的 hiveId
    statusHiveId = "56f96f6b-fcb9-4084-8864-5a9a9377073c";
    maxOccupancy = null;
    userId = "3ECB224C-3993-48F6-8D11-40692884DCF6";
    _statusInterval = null;
    // 不要在类字段初始化时查询 DOM（那时元素尚未创建）。
    // 我们在需要更新 UI 时通过 this.button.querySelector(...) 获取元素。
    fetchedUsername = null;
    room = "";

    constructor() {
      this.isOpen = false;
      this.button = null;
      this.container = null;
      this.iframe = null;
      // 修改为实际的 iframe URL
      this.iframeUrl = "https://vocespace.com/demo_widget";

      // 读取嵌入配置（支持 window.SohiveWidgetConfig, script data-attributes 或 script src query params）
      const cfg = this._readEmbedConfig();
      if (cfg.hiveId) this.statusHiveId = cfg.hiveId;
      if (cfg.userId) this.userId = cfg.userId;

      this.init();
    }

    // 从多种来源读取嵌入时传入的配置
    _readEmbedConfig() {
      const cfg = {};
      try {
        // 1) 全局对象优先
        if (
          window.SohiveWidgetConfig &&
          typeof window.SohiveWidgetConfig === "object"
        ) {
          if (window.SohiveWidgetConfig.hiveId)
            cfg.hiveId = window.SohiveWidgetConfig.hiveId;
          if (window.SohiveWidgetConfig.userId)
            cfg.userId = window.SohiveWidgetConfig.userId;
        }

        // 2) document.currentScript 的 data- 属性
        const cs = document.currentScript;
        if (cs && cs.dataset) {
          if (cs.dataset.hiveId) cfg.hiveId = cs.dataset.hiveId;
          if (cs.dataset.userId) cfg.userId = cs.dataset.userId;
        }

        // 3) 在没有 currentScript 的情况下，查找载入本文件的 script 元素，支持 query string 形式
        if (!cfg.hiveId || !cfg.userId) {
          const scripts = document.getElementsByTagName("script");
          for (let i = scripts.length - 1; i >= 0; i--) {
            const s = scripts[i];
            if (!s.src) continue;
            // 尝试匹配文件名 widget.js（支持带域名或版本号）
            if (s.src.indexOf("widget.js") !== -1) {
              try {
                const url = new URL(s.src, location.href);
                const params = new URLSearchParams(url.search);
                if (!cfg.hiveId && params.get("hiveId"))
                  cfg.hiveId = params.get("hiveId");
                if (!cfg.userId && params.get("userId"))
                  cfg.userId = params.get("userId");
                // 也检查 data-*
                if (!cfg.hiveId && s.dataset && s.dataset.hiveId)
                  cfg.hiveId = s.dataset.hiveId;
                if (!cfg.userId && s.dataset && s.dataset.userId)
                  cfg.userId = s.dataset.userId;
                break;
              } catch (e) {
                // 忽略解析错误
              }
            }
          }
        }
      } catch (e) {
        // 忽略所有读取错误，返回可能为空的 cfg
      }
      return cfg;
    }

    init() {
      // 等待 DOM 加载完成
      if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", () =>
          this.createWidget()
        );
      } else {
        this.createWidget();
      }
    }

    createWidget() {
      this.injectStyles();
      this.createButton();
      // 按钮创建完成后启动状态轮询（合并自 admin.js 的逻辑）
      this.startStatusPolling();
    }

    injectStyles() {
      const style = document.createElement("style");

      style.textContent = `
                .sohive-widget-button {
                    width: 400px;
                    height: 200px;
                    border-radius: 16px;
                    background: transparent;
                    cursor: pointer;
                    border: none;
                    display: flex;
                    align-items: center;
                    justify-content: center;
                    transition: all 0.3s ease;
                }

                .hexagon_shadow {
                    display: flex;
                    justify-content: center;
                    align-items: center;
                    height: 90px;
                    width: 90px;
                }

                .hexagon_shadow > svg{
                    position: absolute;
                    z-index: 1111;
                }
                
                .haxagon_data {
                    position: absolute;
                    z-index: 1121;
                    height: 90px;
                    width: 90px;
                    display: flex;
                    align-items: center;
                    justify-content: center;
                    flex-wrap: wrap;
                    align-content: center;
                }

                .status {
                    width: 100%;
                    display: flex;
                    align-items: center;
                    justify-content: center;
                    gap: 8px;
                }

                .card_wrapper {
                    height: 100%;
                    width: 100%;
                    background-color: #313464;
                    border-radius: 16px;
                    position: relative;
                    padding: 16px;
                    display: flex;
                    gap: 16px;
                    align-items: center;
                    justify-content: space-between;
                }

                .status-dot {
                    width: 10px;
                    height: 10px;
                    border-radius: 50%;
                    display: inline-block;
                    vertical-align: middle;
                   
                }

                .nums {
                    color: #fff;
                }

                .card_left {
                    height: 100%;
                    width: 120px;
                    display: flex;
                    align-items: center;
                    justify-content: center;
                    flex-direction: column;
                    gap: 8px;
                }

                .username {
                  color: #fff;
                  font-size: 16px;
                }

            .card_right {
                flex:1;
                color: #fff;
                height: 116px;
                font-size: 16px;
                display: flex;
                align-items: flex-start;
                justify-content: space-between;
                flex-direction: column;
                gap: 8px;
            }
            `;

      document.head.appendChild(style);
    }

    createButton() {
      this.button = document.createElement("button");
      this.button.className = "sohive-widget-button";
      this.button.setAttribute("aria-label", "打开在线客服");

      this.button.innerHTML = `
            <div class="card_wrapper" id="cardWrapper">
                
                <div class="card_left">
                    <div class="hexagon_shadow">
                        <svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 300" width="100" height="100">
                            <defs>
                                <image  width="248" height="244" id="img1" href="data:image/png;base64,"/>
                            </defs>
                            <style>
                            </style>
                            <use id="Screenshot 2025-11-27 at 9.50.04 AM" href="#img1" x="23" y="25"/>
                        </svg>
                        <div class="haxagon_data">
                            <div class="status" id="status">
                                <svg stroke="#fff" fill="#fff" stroke-width="0" viewBox="0 0 24 24" class="xs5h3dt xettwda" height="16px" width="16px" xmlns="http://www.w3.org/2000/svg"><path d="M24 5.25v13a.75.75 0 0 1-1.136.643L16.5 15.075v2.175A1.75 1.75 0 0 1 14.75 19h-13A1.75 1.75 0 0 1 0 17.25v-11C0 5.284.784 4.5 1.75 4.5h13c.966 0 1.75.784 1.75 1.75v2.175l6.364-3.818A.75.75 0 0 1 24 5.25Zm-9 1a.25.25 0 0 0-.25-.25h-13a.25.25 0 0 0-.25.25v11c0 .138.112.25.25.25h13a.25.25 0 0 0 .25-.25v-11Zm1.5 7.075 6 3.6V6.575l-6 3.6Z"></path></svg>
                                    <span
                                        class="status-dot"
                                        id="statusDot"
                                        style="background: #f44336"
                                    ></span>
                            </div>
                            <div class="nums" id="nums">0 / 0</div>
                        </div>
                    </div>

                    <div class="username" id="fullName">Unknown</div>
                </div>
            </div>
            `;

      this.button.addEventListener("click", () => {
        window.open(`https://sohive.com/hive/${this.room}`, "_blank");
      });
      // document.body.appendChild(this.button);
      document.getElementById("sohive-widget-board").appendChild(this.button);
    }

    toggleWidget() {
      if (this.isOpen) {
        this.closeWidget();
      } else {
        this.openWidget();
      }
    }

    openWidget() {
      // 首次打开时加载 iframe
      if (!this.iframe.src) {
        this.iframe.src = this.iframeUrl;
      }

      this.container.classList.add("open");
      this.isOpen = true;
    }

    closeWidget() {
      this.container.classList.remove("open");
      this.isOpen = false;
    }

    // 公共 API
    open() {
      this.openWidget();
    }

    close() {
      this.closeWidget();
    }

    destroy() {
      if (this.button) {
        this.button.remove();
      }
      if (this.container) {
        this.container.remove();
      }
      if (this._statusInterval) {
        clearInterval(this._statusInterval);
        this._statusInterval = null;
      }
      window.SohiveWidget = null;
    }

    safeJson(resp) {
      if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
      return resp.json();
    }
    async fetchUserName() {
      try {
        const url = `${this.statusBaseUrl}/v1/users/search/${this.userId}`;
        const data = await fetch(url, { cache: "no-store" }).then((r) =>
          this.safeJson(r)
        );
        // 解析用户名字符串
        const name =
          data?.RESPONSE?.searchResults?.[0]?.fullName ??
          data?.response?.searchResults?.[0]?.fullName ??
          "Unknown User";

        // 在按钮范围内查找显示用户名的元素（避免在类字段中提前查询 DOM）
        const el = this.button ? this.button.querySelector("#fullName") : null;
        console.warn("fullName element:", el);
        if (el) {
          el.textContent = name;
        }

        // 同时保存字符串以备后用
        this.fetchedUsername = name;
      } catch (err) {
        console.warn("fetchMaxOccupancy error", err);
        this.fetchedUsername = "Unknown User";
        const el = this.button ? this.button.querySelector("#fullName") : null;
        if (el) el.textContent = this.fetchedUsername;
      }
    }

    async fetchMaxOccupancy() {
      try {
        const url = `${this.statusBaseUrl}/v1/hives/id/${this.statusHiveId}`;
        const data = await fetch(url, { cache: "no-store" }).then((r) =>
          this.safeJson(r)
        );
        this.maxOccupancy =
          data?.RESPONSE?.hivesData?.hives?.[0]?.maxOccupancy ?? null;
        if (this.maxOccupancy == null) {
          this.maxOccupancy =
            data?.response?.hivesData?.hives?.[0]?.maxOccupancy ??
            this.maxOccupancy;
        }
      } catch (err) {
        console.warn("fetchMaxOccupancy error", err);
        this.maxOccupancy = null;
      }
    }

    async fetchUserCount() {
      try {
        const url = `${this.statusBaseUrl}/v1/hives/${this.statusHiveId}/user-count`;
        const data = await fetch(url, { cache: "no-store" }).then((r) =>
          this.safeJson(r)
        );

        const activeHive =
          data?.RESPONSE?.hivesData?.activeHive ??
          data?.response?.hivesData?.activeHive ??
          null;
        const userCount =
          data?.RESPONSE?.hivesData?.hiveInfo?.userCount ??
          data?.response?.hivesData?.hiveInfo?.userCount ??
          null;
        const name =
          data?.RESPONSE?.hivesData?.displayName ??
          data?.response?.hivesData?.displayName ??
          "";
        const drop =
          data?.RESPONSE?.hivesData?.schedule?.[0]?.type === "dropIn"
            ? "Drop-in Anytime"
            : "-";
        const timezone =
          data?.RESPONSE?.hivesData?.timezone ??
          data?.response?.hivesData?.timezone ??
          "";
        this.room = name;
        this.updateStatusUI({ activeHive, userCount, name, drop, timezone });
      } catch (err) {
        console.warn("fetchUserCount error", err);
        // 出错时把状态设为离线（或保留原样），这里设为离线并显示 --
        this.updateStatusUI({ activeHive: false, userCount: null });
      }
    }

    updateStatusUI({ activeHive, userCount, name, drop, timezone }) {
      // 找到按钮内的元素（优先在 widget 内查找，避免与页面其它元素冲突）
      const statusDot = this.button
        ? this.button.querySelector("#statusDot")
        : null;
      const nums = this.button ? this.button.querySelector("#nums") : null;

      const nameEl = this.button ? this.button.querySelector("#name") : null;
      const dropEl = this.button ? this.button.querySelector("#drop") : null;
      const timezoneEl = this.button
        ? this.button.querySelector("#timezone")
        : null;

      if (nameEl) {
        nameEl.textContent = name || "Name";
      }
      if (dropEl) {
        dropEl.textContent = drop || "Drop";
      }
      if (timezoneEl) {
        timezoneEl.textContent = `TimeZone:${timezone || "TimeZone"}`;
      }
      if (statusDot) {
        if (activeHive === true || activeHive === "true" || activeHive === 1) {
          statusDot.style.background = "#2ecc71";
        } else {
          statusDot.style.background = "#f44336";
        }
      }

      if (nums) {
        const displayUser =
          userCount == null || isNaN(userCount) ? "0" : String(userCount);
        const displayMax =
          this.maxOccupancy == null ? "0" : String(this.maxOccupancy);
        nums.textContent = `${displayUser} / ${displayMax}`;
      }
    }

    async startStatusPolling() {
      // 先获取最大容量，再开始轮询人数
      await this.fetchMaxOccupancy();
      await this.fetchUserCount();
      await this.fetchUserName();
      // 每 30 秒轮询人数
      this._statusInterval = setInterval(
        () => this.fetchUserCount(),
        30 * 1000
      );
    }

    // 运行时 API：动态设置 hiveId（会触发一次刷新）
    setHiveId(hiveId) {
      if (!hiveId) return;
      this.statusHiveId = hiveId;
      // 重新拉取相关数据
      this.fetchMaxOccupancy();
      this.fetchUserCount();
    }

    // 运行时 API：动态设置 userId（会触发一次用户名刷新）
    setUserId(userId) {
      if (!userId) return;
      this.userId = userId;
      this.fetchUserName();
    }
  }

  // 初始化 Widget
  const widget = new SohiveWidget();

  // 暴露全局 API
  window.SohiveWidget = {
    open: () => widget.open(),
    close: () => widget.close(),
    destroy: () => widget.destroy(),
    // runtime setters
    setHiveId: (id) => widget.setHiveId(id),
    setUserId: (id) => widget.setUserId(id),
    // for debugging/read
    getConfig: () => ({ hiveId: widget.statusHiveId, userId: widget.userId }),
  };
})();
